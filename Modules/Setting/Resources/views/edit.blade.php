        ...
        @foreach ($Models as $Setting)
            ...
            @elseif (str_contains($Setting['key'], '_services') || (str_contains($Setting['key'], 'ar') || str_contains($Setting['key'], 'en') || $Setting['type'] == 'meta') && !str_contains($Setting['key'], 'title_'))
                <div class="col-sm-12">
                    <div class="form-group">
                        <label for="{{ $Setting['key'] }}">@lang('trans.'.$Setting['key'])</label>
                        <textarea style="min-height: 300px" class="form-control" id="{{ $Setting['key'] }}" name="{{ $Setting['key'] }}"  placeholder="@lang('trans.' . $Setting['key'])">{!! $Setting['value'] !!}</textarea>
                    </div>
                </div>
            ...
            @else
                <div class="col-sm-12">
                    <div class="form-group">
                        <label for="{{ $Setting['key'] }}">@lang('trans.'.$Setting['key'])</label>
                        <input id="{{ $Setting['key'] }}" type="text" name="{{ $Setting['key'] }}" {{$Setting['key'] == 'whatsapp' ? '' : 'required'}}  placeholder="@lang('trans.' . $Setting['key'])" class="form-control" value="{{ $Setting['value'] }}">
                    </div>
                </div>
            @endif
        @endforeach
