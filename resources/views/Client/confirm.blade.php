                                    ...
                                    <div class="flex-grow-1 align-self-center overflow-hidden">
                                        <div>
                                            ...
                                            @forelse($CartItem->typeItem as $item)
                                                @php($CalcPrice += $item->CalcPrice())
                                                @php($itemsPrice += $item->CalcPrice())
                                                <p class="mb-0 mt-1">@lang('trans.item') : <span class="fw-medium">{{ $item->title() }} ({{ $item->CalcPriceWithCurrancy() }})</span></p>
                                            @empty
                                            @endforelse
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-3 col-md-3">
                                            <div class="mt-3">
                                                ...
                                                <div class="d-inline-flex">
                                                    <div class="number">
                                                        <input data-id="{{ $CartItem->id }}" data-price="{{ $CalcPrice }}" data-discount="{{ $Price -  $CalcPrice + ($itemsPrice)  }}" type="text" class="form-control border-0" value="{{ $CartItem->quantity }}" readonly />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="mt-5 mt-lg-0">
                                <div class="card border shadow-none" style="text-align: inherit">
                                    <div class="card-header bg-transparent border-bottom py-3 px-4">
                                        <h5 class="font-size-16 mb-0">@lang('trans.coupon')</h5>
                                    </div>
                                                    ...
                                                    <td>
                                                        <div class="form-group">
                                                            ...
                                                            <p class="form-text text-muted">
                                                                @lang('trans.If_you_have_coupon_code')
                                                            </p>
                                                        </div>
                                                        <button type="button" class="btn main_btn float-end" id="coupon_apply">@lang('trans.apply')</button>
                                                    </td>
                                                </tr>
                                                </tbody>
                                            </table>
                            ...
                            <div class="card-body p-4 pt-2">
                                            ...
                                            @foreach (auth('client')->user()->Addresses()->get() as $Address)
                                                <div class="col-md-6">
                                                    ...
                                                    <label class="form-check-label" for="del-{{ $Address->id }}">
                                                        ...
                                                        @endif
                                                        @if ($Address->floor_no)
                                                            <p class="m-0">@lang('trans.floor_no') : {{ $Address->floor_no }}</p>
                                                        @endif
                                ...
                                @else
                                    ...
                                    @else
                                        ...
                                        <input type="hidden" name="branch_id" id="del-{{ $Branch->id }}" value="{{ $Branch->id }}">
                                    @endif
                                @endif
                            </div>
                        </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table mb-0">
                                        <tbody>
                                            ...
                                            @else
                                                @php($Payments = Payments()->whereNotIn('id',[6]))
                                            @endif
                                            @foreach ($Payments as $Payment)
                                            <tr>
                                                <td>
                                                    <div class="form-check form-check-inline" id="Payment-{{ $Payment->id }}">
                                                        <input class="form-check-input" style="float: inline-start;margin: 15px 10px;" type="radio" name="payment_id" id="del-{{ $Payment->id }}" value="{{ $Payment->id }}" @checked($loop->first)>
                                ...
                                <div class="table-responsive">
                                    <table class="table mb-0">
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>@lang('trans.coupon') (<span id="coupon_percentage">0%</span>) : </td>
                                                <td class="text-end">- {{ Country()->currancy_code }} <span id="coupon">0</span> </td>
                                            </tr>
                                            <tr>
                                                <td>@lang('trans.coins') (<span id="coins">{{ auth('client')->user()->coins }}</span>) :  </td>
                                                <td class="text-end"> {{ Country()->currancy_code }} <span id="coins">{{format_number( (((setting('discount_one_coin') * auth('client')->user()->coins) / setting('coins_no')) * Country()->currancy_value))}}</span> </td>
                                            </tr>
                                            @if (request('delivery_id') == 1)
                                                <tr>
    ...
    <script>
        ...
        $(document).on("click", "#coupon_apply", function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.type === 'success' && data.coupon != null) {
                        $('<input>').attr({
                            type: 'hidden',
                            id: 'extraData',
                            name: 'extra_data',
                        ...
                        coupon_discount = data.coupon.discount_amount;
                        calc();
                    }
                }
            });
        ...
        function calc(){
            ...
            if(coupon_discount > 0){
                coupon = sub_total / 100 * coupon_discount;
            }
            var coins_value = {{format_number((((setting('discount_one_coin') * auth('client')->user()->coins) / setting('coins_no')) * Country()->currancy_value))}};
            console.log(coins_value);
            vat = sub_total/100*vat_percentage;
            netTotal = (sub_total + vat + delivery_charge) - (coupon + coins_value);
            sub_total += discount;
            $('#sub_total').html(sub_total.toFixed(decimals));
            $('#discount').html(discount.toFixed(decimals));
            $('#vat').html(vat.toFixed(decimals));
            $('#coupon').html(coupon.toFixed(decimals));
            $('#coupon_percentage').html(coupon_discount.toFixed(0) + '%');
            @if (request('delivery_id') == 1)    
                $('#delivery_charge').html(delivery_charge.toFixed(decimals));
            @endif
            if(netTotal > {{ auth('client')->user()?->wallet ?? 0 }}){
                $('#Payment-6').hide();
            }
