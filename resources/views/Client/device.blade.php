                    ...
                    <div class="col-md-7 col-sm-12">
                        <div class="white-box text-center">
                            <div class="img__wrapper">
                                @if(request('color_id') && $Device->Headers()->count())
                                    ...
                                    @endif
                                @else
                                    @if(IsVideo($Device->header) && $Device->Headers()->count())
                        ...
                        <div class="carousel-inner">
                            @foreach ($Device->Gallery->when(request('color_id'), fn($query) =>  $query->where('color_id', request('color_id')) ) as $Slider)
                                <div class="carousel-item py-3 text-center {{ $loop->first ? ' active' : '' }}">
                                    <img loading="lazy" src="{{ $Slider->image }}" alt="SliderImage" style="max-height: 500px;max-width:100%">
                                </div>
                            @endforeach
                        </div>
                        @if ($Device->Gallery->when(request('color_id'), fn($query) =>  $query->where('color_id', request('color_id')) )->count() > 1)
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            text: data.message,
                            confirmButtonColor: '{{setting('main_color')}}',
                        });
